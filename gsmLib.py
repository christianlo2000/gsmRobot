...
def connectGSMData(ser, apn):
    ...
    # Returning all messages from modem
    reply = ser.read(ser.inWaiting())
    debug("connectGSMData() retured:\n" + reply)
    return reply
# para desconectar modem
def disconnectGSMData(ser):
     # desconectamos PDP session
    cmd = 'AT+CGACT=0,1\r'
    debug("Cmd: "+cmd)
    ser.write(cmd)
    time.sleep(3)
    # desconectamos de red datos GPRS
    cmd = 'AT+CGATT=0\r'
    debug("Cmd: "+cmd)
    ser.write(cmd)
    time.sleep(3)
...
# conexion a website
def connect2Web(ser,url,byteAmount):
    cmd = 'AT+CIPMUX=0\r'
    debug("Cmd: "+cmd)
    ser.write(cmd)
    time.sleep(5)
    ...
    cmd = 'AT+CIPSTART="TCP",url,80\r'
    debug("Cmd: "+cmd)
    ser.write(cmd)
    time.sleep(6)
    # cantidad de bytes que me traigo
    cmd = 'AT+CIPSEND='+str(byteAmount)+'\r'
    debug("Cmd: "+cmd)
    ser.write(cmd)
    # leemos que devuelve el mode
    time.sleep(5)
    print(ser.read())
